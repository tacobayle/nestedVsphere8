...
resource "vsphere_distributed_virtual_switch" "sa_network_nsx_external" {
  count = (var.vsphere_underlay.networks_vsphere_dual_attached == false) && (var.deployment == "vsphere_nsx" || var.deployment == "vsphere_nsx_alb" || var.deployment == "vsphere_nsx_alb_telco" || var.deployment == "vsphere_nsx_alb_vcd") ? 1 : 0
