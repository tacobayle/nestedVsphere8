...
resource "null_resource" "add_ips_to_gw_alb_tanzu" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo \"            dhcp4: false\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      "echo \"            addresses: [${var.vsphere_underlay.networks.alb.se.external_gw_ip}/${split("/", var.vsphere_underlay.networks.alb.se.cidr)[1]}]\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      "echo \"            match:\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      ...
      "echo \"            dhcp4: false\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      "echo \"            addresses: [${var.vsphere_underlay.networks.alb.backend.external_gw_ip}/${split("/", var.vsphere_underlay.networks.alb.backend.cidr)[1]}]\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      "echo \"            match:\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      ...
      "echo \"            dhcp4: false\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      "echo \"            addresses: [${var.vsphere_underlay.networks.alb.vip.external_gw_ip}/${split("/", var.vsphere_underlay.networks.alb.vip.cidr)[1]}]\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      "echo \"            match:\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      ...
      "echo \"            dhcp4: false\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      "echo \"            addresses: [${var.vsphere_underlay.networks.alb.tanzu.external_gw_ip}/${split("/", var.vsphere_underlay.networks.alb.tanzu.cidr)[1]}]\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
      "echo \"            match:\" | sudo tee -a /etc/netplan/50-cloud-init.yaml",
