...
data "template_file" "external_gw_userdata_nsx_alb" {
...
resource "vsphere_virtual_machine" "external_gw_nsx_alb" {
  count = var.deployment == "vsphere_tanzu_nsx_alb" ? 1 : 0
...
//  }
  ...
  vapp {
    properties = {
      ...
      user-data   = base64encode(data.template_file.external_gw_userdata_nsx_alb[0].rendered)
    }
  }
...
resource "null_resource" "add_nic_to_gw_network_nsx_external" {
  depends_on = [vsphere_virtual_machine.external_gw_nsx_alb]
  count = var.deployment == "vsphere_tanzu_nsx_alb" ? 1 : 0
...
resource "null_resource" "update_ip_routes" {
  ...
  count = var.deployment == "vsphere_tanzu_nsx_alb" ? length(var.external_gw.routes) : 0
...
resource "null_resource" "update_ip_tables" {
  ...
  count = var.deployment == "vsphere_tanzu_nsx_alb" ? length(var.external_gw.ip_table_prefixes) : 0
