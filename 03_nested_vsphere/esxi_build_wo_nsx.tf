resource "vsphere_virtual_machine" "esxi_host" {
  ...
  count = var.deployment == "vsphere_wo_nsx" && var.vsphere_underlay.networks_vsphere_dual_attached == true ? length(var.vsphere_underlay.networks.vsphere.management.esxi_ips) : 0
...
resource "vsphere_virtual_machine" "esxi_host_single_attached" {
  depends_on = [ vsphere_file.iso_upload ]
