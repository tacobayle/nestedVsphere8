...
do
  govc dvs.add -dvs "$(jq -r .networks.vsphere.VSAN.vds_name $jsonFile)" -pnic=vmnic3 "$(jq -r .vsphere_nested.esxi.basename $jsonFile)$count.$(jq -r .external_gw.bind.domain $jsonFile)"
  count=$((count+1))
done
#