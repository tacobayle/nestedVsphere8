resource "vsphere_virtual_machine" "esxi_host_tanzu" {
  depends_on = [ vsphere_file.iso_upload ]
  count = var.deployment == "vsphere_alb_wo_nsx" && var.vsphere_underlay.networks.vsphere.dual_attached == true  ? length(var.vsphere_underlay.networks.vsphere.management.esxi_ips) : 0
...
resource "vsphere_virtual_machine" "esxi_host_tanzu_single_attached" {
  depends_on = [ vsphere_file.iso_upload ]
