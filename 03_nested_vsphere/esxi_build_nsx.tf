resource "vsphere_virtual_machine" "esxi_host_nsx" {
  ...
  count = var.deployment != "vsphere_wo_nsx" && var.deployment != "vsphere_alb_wo_nsx" && var.vsphere_underlay.networks.vsphere.dual_attached == true ? length(var.vsphere_underlay.networks.vsphere.management.esxi_ips) : 0
