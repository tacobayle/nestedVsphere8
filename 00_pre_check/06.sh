...
if [[ $(jq -c -r .nsx $jsonFile) != "null" ]]; then
  ...
  ip_pool_0=$(echo $ip_pool | jq '. += {"gateway": "'$(jq -c -r .vcenter_underlay.networks.nsx.overlay.nsx_pool.gateway $jsonFile)'"}')
  ip_pool_0=$(echo $ip_pool | jq '. += {"start": "'$(jq -c -r .vcenter_underlay.networks.nsx.overlay.nsx_pool.start $jsonFile)'"}')
  ip_pool_0=$(echo $ip_pool | jq '. += {"end": "'$(jq -c -r .vcenter_underlay.networks.nsx.overlay.nsx_pool.end $jsonFile)'"}')
  ip_pool_0=$(echo $ip_pool | jq '. += {"cidr": "'$(jq -c -r .vcenter_underlay.networks.nsx.overlay.nsx_pool.cidr $jsonFile)'"}')
  ip_pools=$(echo $ip_pools | jq '. += [{"'$(echo $ip_pool_0 | jq -c -r .)'"}]')
  ip_pool_1=$(jq -c -r '.ip_pools[0]' $localJsonFile)
  ip_pool_1=$(echo $ip_pool | jq '. += {"gateway": "'$(jq -c -r .vcenter_underlay.networks.nsx.overlay_edge.nsx_pool.gateway $jsonFile)'"}')
  ip_pool_1=$(echo $ip_pool | jq '. += {"start": "'$(jq -c -r .vcenter_underlay.networks.nsx.overlay_edge.nsx_pool.start $jsonFile)'"}')
  ip_pool_1=$(echo $ip_pool | jq '. += {"end": "'$(jq -c -r .vcenter_underlay.networks.nsx.overlay_edge.nsx_pool.end $jsonFile)'"}')
  ip_pool_1=$(echo $ip_pool | jq '. += {"cidr": "'$(jq -c -r .vcenter_underlay.networks.nsx.overlay_edge.nsx_pool.cidr $jsonFile)'"}')
  ip_pools=$(echo $ip_pools | jq '. += [{"'$(echo $ip_pool_1 | jq -c -r .)'"}]')
  ...
  nsx_json=$(echo $nsx_json | jq '. += {"nsx.config.uplink_profiles": '$(echo $uplink_profiles)'}')
  #
  echo "   +++ Adding transport_zones details..."
  transport_zones=$(jq -c -r '.transport_zones' $localJsonFile)
  nsx_json=$(echo $nsx_json | jq '. += {"nsx.config.transport_zones": '$(echo $transport_zones)'}')
  #
  echo "   +++ Adding segments details..."
  segments=$(jq -c -r '.segments' $localJsonFile)
  nsx_json=$(echo $nsx_json | jq '. += {"nsx.config.segments": '$(echo $segments)'}')
  #
  echo "   +++ Adding transport_node_profiles details..."
  transport_node_profiles=$(jq -c -r '.transport_node_profiles' $localJsonFile)
  nsx_json=$(echo $nsx_json | jq '. += {"nsx.config.transport_node_profiles": '$(echo $transport_node_profiles)'}')
  #
  ...
  nsx_json=$(echo $nsx_json | jq '. += {"nsx.config.edge_node.host_switch_spec": '$(echo $host_switch_spec)'}')
  #
  ...
  do
    ...
    do
      ...
      if [[ $(echo $item | jq -c -r .edge_cluster_name) == $(echo $edge_cluster | jq -c -r .display_name) ]] ; then
        count=0
        for edge in $(echo $$edge_cluster | jq -c -r .members_name[] $jsonFile)
        do
          interface="{}"
          interface=$(echo $interface | jq '. += {"edge_name": "'$(jq $edge)'"}')
    ...
    item=$(echo $item | jq '. += {"interfaces": "'$(echo $interfaces)'"}')
    item=$(echo $item | jq '. += {"static_routes": "'$(echo $static_routes)'"}')
    ha_vips=[]
    ha_vips=$(echo $ha_vips | jq '. += [{"interfaces": "'$(echo $ha_vips_interfaces | jq -c -r .)'"}]')
    item=$(echo $item | jq '. += {"ha_vips": "'$(echo $ha_vips)'"}')
    tier0s=$(echo $tier0s | jq '. += [{"'$(echo $item | jq -c -r .)'"}]')
  done
  #
  echo "   +++ Adding tier1s details..."
  tier1s=[]
  route_advertisement_types=$(jq -c -r '.tietier1sr0s.route_advertisement_types' $localJsonFile)
  for item in $(jq -c -r .nsx.config.tier1s[] $jsonFile)
  do
    item=$(echo $item | jq '. += {"route_advertisement_types": "'$(echo $route_advertisement_types)'"}')
  done
  tier1s=$(echo $tierqs | jq '. += [{"'$(echo $item | jq -c -r .)'"}]')
  #
